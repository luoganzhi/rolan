...
- 搜索、拖拽添加、浏览添加文件/文件夹；拖入多行路径/URL/系统命令会批量添加
- 搜索框无选中文本时可用 `Ctrl + V` 从剪贴板粘贴文件、文件夹、路径、URL 或系统命令到当前分组
- 手动添加文件路径、文件夹路径、URL
...
- 搜索框无匹配项时可直接回车打开输入的 URL、真实路径或内置系统命令
- 搜索会按空格分词匹配名称、目标路径、启动参数、工作目录、快捷方式类型和中文首字母，搜索结果会显示来源分组
- 分组栏提供“常用”视图，按最近启动和启动次数展示高频快捷方式
- 面板获得焦点后可直接键入开始搜索
- `Ctrl + F` 可随时聚焦并全选搜索框
- 搜索框和快捷方式网格支持上下键、PageUp/PageDown、Home/End 快速切换选中项，Enter 启动当前选中项
- `Alt + 1..9` 可直接启动当前列表第 1 到第 9 个快捷方式
- 搜索框无选中文本时和快捷方式网格支持 F2 编辑、Ctrl+C 复制目标、Ctrl+Shift+C 复制完整命令行、Ctrl+D 创建副本、Ctrl+O 打开文件位置；快捷方式网格支持 Delete 删除，搜索框为空时 Delete 删除当前选中项
- 快捷方式编辑、创建副本、删除、移动、排序，支持拖拽重排和跨分组移动
- 删除快捷方式会二次确认，避免误删
- 右键菜单会按快捷方式类型禁用无意义操作，并支持复制目标和完整命令行
- 鼠标悬停快捷方式会显示目标、参数、工作目录、分组和使用记录摘要
- 可重置单个快捷方式的启动次数和最近启动时间
- 支持编辑启动参数和工作目录
...
- 设置和关于窗口会挂靠主面板显示，避免在置顶或贴边场景中跑到背后
- 面板可按当前分组或搜索结果自动调整高度，并受屏幕工作区限制
- 开机自启到托盘、可配置全局热键
...
- 默认 `Alt + Space`
- `Ctrl + 1` 切换到“常用”，`Ctrl + 2..9` 切换到对应分组
- 可在设置窗口调整热键组合
